...
public class MinimumSpanningTree {
	...
	class Cmp implements Comparator<Connection> {
		public int compare(Connection c1, Connection c2) {
	...
	public ArrayList<Connection> getLowCost(ArrayList<Connection> connections) {
		...
		connections.sort(new sort_connections());
		while (connections.size() != 0) {
			Connection nextEdge = findMin(connections, cityTree);
			if (nextEdge == null) {
				break;
			}
			cityTree.add(nextEdge.node1);
			cityTree.add(nextEdge.node2);
			connections.remove(nextEdge);
			res.add(nextEdge);
		}
		res.sort(new Cmp());
	...
	public int numberOfCity (ArrayList<Connection> graph) {
